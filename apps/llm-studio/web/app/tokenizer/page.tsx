...
const LEGACY_TOKENIZER_API_SEGMENT = "apps/tokenizer-studio/api/";
const CURRENT_TOKENIZER_API_SEGMENT = "apps/llm-studio/api/";
const LEGACY_UPLOADS_SEGMENT = "/datasets/uploads/";
const DEFAULT_SHAKE_DATASET_PATH = "datasets/shake.txt";
...
function migrateLegacyLocalTrainFilePath(value: string): string {
  ...
  if (normalized.includes(LEGACY_UPLOADS_SEGMENT)) {
    const normalizedFileName = stripGeneratedUploadPrefix(
...
function resolveLocalTrainFilePaths(files: LocalTrainFileFormState[]): string[] {
...
function hydrateDatasetForm(value: unknown, fallback: DatasetFormState): DatasetFormState {
  ...
  const legacyTrainFilePath = migrateLegacyLocalTrainFilePath(
    asString(record.trainFilePath, "")
  );
