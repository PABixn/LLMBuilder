...
class RunPodClient:
    ...
    def _request(self, method: str, path: str, *, json_body: dict[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            payload = _sanitize_error_payload(_decode_error_payload(exc))
            detail = _error_message(payload)
            message = (
                f"RunPod request failed with HTTP {exc.code}: {detail}"
                if detail
                else f"RunPod request failed with HTTP {exc.code}."
            )
