...
class TrainingRunManager:
    ...
    def _resolve_preflight_context(self, request: TrainingPreflightRequest) -> ResolvedPreflightContext:
        ...
        if parsed_training_config is not None:
            fixes.append(
                TrainingFixSuggestion(
                    code="load_starter_optimizer_defaults",
                    label="Load starter optimizer defaults",
                    description="Reset the optimizer section to the repo starter defaults.",
                    path="training_config.optimizer",
                    value=load_json(IMPORT_ROOT / "training" / "training_config.json")["optimizer"],
                )
            )
            fixes.append(
                TrainingFixSuggestion(
                    code="load_starter_scheduler_template",
                    label="Load starter scheduler template",
                    description="Reset the LR scheduler to the repo starter template.",
                    path="training_config.lr_scheduler",
                    value=load_json(IMPORT_ROOT / "training" / "training_config.json")["lr_scheduler"],
                )
            )
        ...
        if parsed_training_config is not None and parsed_dataloader_config is not None:
            ...
            if parsed_training_config.save_every > parsed_training_config.max_steps:
                warnings.append(
                    issue(
                        "save_every_exceeds_max_steps",
                        "save_every is larger than max_steps, so only the final checkpoint will be saved.",
